...
unsplc_reads_WT_rep1 <- Miquel_1 %>% mutate(genotype="WT", bio_rep="1")
unsplc_reads_WT_rep2 <- Miquel_2 %>% mutate(genotype="WT", bio_rep="2")
unsplc_reads_WT_rep3 <- Miquel_3 %>% mutate(genotype="WT", bio_rep="3")
unsplc_reads_mut_rep1 <- Miquel_4 %>% mutate(genotype="mut", bio_rep="1")
unsplc_reads_mut_rep2 <- Miquel_5 %>% mutate(genotype="mut", bio_rep="2")
unsplc_reads_mut_rep3 <- Miquel_6 %>% mutate(genotype="mut", bio_rep="3")
...
# Ploting
```{r}
# Prepare datasets for shading intersection using the function stat_difference() from the ggh4x package
...
ggplot(unsplc_reads_ratio_rollmed_avg, aes(read_end, rollmed_avg, group=genotype)) + geom_bar(aes(fill=genotype), stat = "identity", position = position_dodge(), alpha=0.2) + geom_line(aes(color=genotype)) + xlim(18588549, 18591464)
```
